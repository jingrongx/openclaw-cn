...
export const zhCN = {
    ...
    configure: {
        ...
        prompts: {
            authProvider: "模型/认证提供商 (Model/auth provider)",
            authMethod: "{provider} 认证方式",
            noAuthAvailable: "该提供商通过没有可用的认证方式。",
            back: "返回 (Back)",
            configureSkills: "是否现在配置技能? (推荐)",
            installMissingSkills: "安装缺失的技能依赖",
            skipForNow: "暂时跳过 (Skip for now)",
            continueWithoutDeps: "继续但不安装依赖",
            preferredNodeManager: "首选的 Node 包管理器 (用于安装技能)",
            installingSkill: "正在安装 {skill}…",
            installedSkill: "已安装 {skill}",
            installFailed: "安装失败: {skill}",
            setEnv: "是否为 {skill} 设置 {env}?",
            enterEnv: "请输入 {env}",
            enableHooks: "启用钩子 (Hooks)?",
            noHooks: "未发现可用的钩子。您稍后可以在配置中设置。",
            hooksConfigured: "已启用 {count} 个钩子: {names}",
            configureChannels: "是否现在配置聊天渠道?",
            selectChannel: "选择一个渠道",
            selectChannelQuickstart: "选择渠道 (快速上手)",
            channelStatus: "渠道状态",
            alreadyConfigured: "{label} 已配置。您想做什么？",
            modifySettings: "修改设置",
            disableKeepConfig: "禁用 (保留配置)",
            deleteConfig: "删除配置",
            skipLeaveAsIs: "跳过 (保持原样)",
            configureDmPolicy: "是否现在配置私聊 (DM) 访问策略? (默认: 配对模式)",
            dmPolicy: "{label} DM 策略",
            pairingRecommended: "配对模式 (Pairing - 推荐)",
            allowlist: "白名单 (Allowlist - 仅限特定用户)",
            publicOpen: "公开 (Open - 允许所有人)",
